...
export default function ProductInfo({ product }) {
    ...
    return (<div className="flex flex-col pt-0 lg:pt-2">
      ...
      <div className="border-t border-gray-100 pt-5 space-y-5">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {product.colors.map((color) => (<button key={color.name} onClick={() => setSelectedColor(color.name)} title={color.name} className={`w-8 h-8 rounded-full border-2 transition-all duration-150 ${selectedColor === color.name
                ? "border-charcoal ring-2 ring-charcoal ring-offset-2 scale-110"
                : "border-gray-200 hover:border-gray-400"}`} style={{ backgroundColor: color.hex }}/>))}
          </div>
        </div>

        {/* Size selector */}
        <ProductSizeSelector sizes={product.sizes} selected={selectedSize} onSelect={(s) => { setSelectedSize(s); setSizeError(false); }} error={sizeError}/>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center border border-gray-200">
              <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center text-charcoal hover:bg-gray-50 transition-colors font-medium text-lg" aria-label="Decrease">
                −
              </button>
              <span className="w-10 text-center font-body text-sm font-medium select-none">
                {quantity}
              </span>
              <button onClick={() => setQuantity((q) => Math.min(10, q + 1))} className="w-10 h-10 flex items-center justify-center text-charcoal hover:bg-gray-50 transition-colors font-medium text-lg" aria-label="Increase">
                +
              </button>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button onClick={handleAddToBag} className="flex-1 h-12 bg-charcoal text-white font-body text-[12px] tracking-[0.18em] font-semibold hover:bg-charcoal/85 active:scale-[0.99] transition-all relative overflow-hidden">
            ...
            <button onClick={() => toggleWishlist(product.id)} className={`w-12 h-12 border flex items-center justify-center transition-all duration-200 ${wishlisted
            ? "border-red-400 bg-red-50"
            : "border-gray-200 hover:border-charcoal"}`} aria-label="Add to wishlist">
              <Heart size={18} strokeWidth={1.5} className={wishlisted ? "fill-red-500 text-red-500" : "text-charcoal"}/>
            </button>

            <button className="w-12 h-12 border border-gray-200 hover:border-charcoal flex items-center justify-center transition-colors" aria-label="Share">
        ...
        {/* Size error message */}
        <AnimatePresence>
          {sizeError && (<motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="font-body text-xs text-red-600 font-medium -mt-2">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 p-3 lg:hidden shadow-lg">
        <button onClick={handleAddToBag} className="w-full h-12 bg-charcoal text-white font-body text-[12px] tracking-[0.18em] font-semibold hover:bg-charcoal/85 transition-colors relative overflow-hidden">
          <AnimatePresence mode="wait">
            {added ? (<motion.span key="a" initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -16, opacity: 0 }} className="block">
                ✓ ADDED TO BAG
              </motion.span>) : (<motion.span key="b" initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -16, opacity: 0 }} className="block">
