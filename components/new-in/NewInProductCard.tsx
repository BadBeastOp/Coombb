...
import { NewInProduct } from "./NewInData";
...
export default function NewInProductCard({ product, priority }: Props) {
    ...
    >
      ...
      >
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
          {product.badge === "NEW" && (
            <span className="font-body text-[9px] tracking-[0.12em] bg-charcoal text-white px-2 py-0.5 font-medium">
              NEW
            </span>
          )}
          {product.badge === "SALE" && (
            <span className="font-body text-[9px] tracking-[0.12em] bg-red-600 text-white px-2 py-0.5 font-medium">
              SALE
            </span>
          )}
          {discount > 0 && (
            <span className="font-body text-[9px] bg-red-600 text-white px-2 py-0.5 font-medium">
              -{discount}%
            </span>
          )}
        </div>
        ...
        {/* Next Day badge */}
        {product.nextDay && (
          <div className="absolute bottom-10 left-0 right-0 flex justify-center z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        ...
        <button
          ...
          className="absolute top-2 right-2 w-8 h-8 bg-white/90 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 hover:bg-white z-10 shadow-sm"
