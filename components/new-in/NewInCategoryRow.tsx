...
export default function NewInCategoryRow({ onSelect, activeId }: Props) {
        ...
        <div className="flex gap-5 sm:gap-8 overflow-x-auto scrollbar-hide justify-start lg:justify-center">
          {CATEGORY_CIRCLES.map((cat) => (
            <button
              ...
              className="flex flex-col items-center gap-2.5 flex-none group"
            >
              {/* Circle */}
              <div className={`relative w-[68px] h-[68px] sm:w-[80px] sm:h-[80px] rounded-full overflow-hidden transition-all duration-200 ${
                active === cat.id
              ...
              {/* Label */}
              <span className={`font-body text-[10px] sm:text-[11px] text-center leading-tight w-[76px] sm:w-[88px] transition-colors ${
                active === cat.id
