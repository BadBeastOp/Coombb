      ...
      <div className={`relative z-10 flex flex-col justify-end h-full px-6 sm:px-12 lg:px-20 pb-14 sm:pb-20 ${contentAlign}`}>
        <AnimatePresence mode="wait">
          <motion.div key={`c-${slide.id}`}
            ...
            <h1 className="font-display font-light text-white leading-none mb-1"
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
        {SLIDES.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)}
            className={`transition-all duration-300 rounded-full ${i === current ? "w-6 h-1.5 bg-white" : "w-1.5 h-1.5 bg-white/35 hover:bg-white/65"}`} />
        ))}
      </div>

      {!paused && (
        <div className="absolute bottom-0 left-0 right-0 z-20 h-0.5 bg-white/10">
          <motion.div key={`p-${current}`} className="h-full bg-white/50"
