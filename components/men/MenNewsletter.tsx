...
export default function MenNewsletter() {
  ...
  return (
    <section className="py-16 sm:py-20 lg:py-24 bg-charcoal text-white">
      <div className="max-w-xl mx-auto px-6 text-center">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <p className="font-body text-sm text-white/40 mb-9 leading-relaxed">
            Join the MAISON mens community for style drops, early access and exclusive offers.
          </p>
          <AnimatePresence mode="wait">
            {!submitted ? (
              <motion.form key="form" exit={{ opacity: 0, y: -10 }} onSubmit={handleSubmit}
                className="flex flex-col xs:flex-row max-w-md mx-auto">
                <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                  placeholder="Your email address"
                  className="flex-1 min-w-0 bg-white/5 border border-white/15 border-r-0 px-5 py-3.5 font-body text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/40 transition-colors" />
                <button type="submit" disabled={loading}
                  className="flex-none bg-white text-charcoal px-6 py-3.5 font-body text-[11px] tracking-[0.2em] hover:bg-bone transition-colors disabled:opacity-70 whitespace-nowrap font-semibold border border-white">
                  {loading ? "..." : "SUBSCRIBE"}
                </button>
              </motion.form>
            ) : (
