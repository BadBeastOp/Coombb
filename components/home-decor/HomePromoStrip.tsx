...
export default function HomePromoStrip() {
  ...
  return (
    <div className="w-full bg-[#2a2a2a] text-white overflow-hidden">
      <div className="relative h-9 flex items-center justify-center px-4">
        {MESSAGES.map((msg, i) => (
          <Link key={i} href={msg.href}
            className={`absolute inset-0 flex items-center justify-center font-body text-[11px] sm:text-xs tracking-[0.22em] font-medium transition-all duration-500 hover:text-bone ${
