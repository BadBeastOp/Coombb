      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10">
        <div className="flex items-end justify-between mb-7 sm:mb-9">
          ...
          <Link href="/home-decor"
            className="hidden sm:block font-body text-xs tracking-[0.2em] text-stone hover:text-charcoal transition-colors border-b border-stone/40 hover:border-charcoal pb-0.5">
            VIEW ALL →
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
          ...
          {CATS.slice(1, 3).map((cat, i) => (
            <motion.div key={cat.id}
              initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ duration: 0.55, delay: (i + 1) * 0.07 }}>
              <CatCard item={cat} height="h-[160px] sm:h-[204px]" />
            </motion.div>
          ))}

          {CATS.slice(3, 5).map((cat, i) => (
            <motion.div key={cat.id}
              initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ duration: 0.55, delay: (i + 3) * 0.07 }}>
              <CatCard item={cat} height="h-[160px] sm:h-[212px]" />
            </motion.div>
          ))}

          {CATS.slice(5).map((cat, i) => (
            <motion.div key={cat.id}
              initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ duration: 0.55, delay: (i + 5) * 0.07 }}>
...
function CatCard({ item, height }: { item: typeof CATS[0]; height: string }) {
  return (
    <Link href={item.href}
