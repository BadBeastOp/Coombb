        ...
        <div className="px-4 sm:px-6 lg:px-8 mb-6 flex items-end justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => scroll("left")} aria-label="Scroll left"
              className="w-9 h-9 border border-gray-200 flex items-center justify-center hover:border-charcoal hover:bg-charcoal hover:text-white transition-colors">
        ...
        <div ref={scrollRef}
          ...
          {products.map((product, i) => (
            <motion.div key={product.id}
              ...
              className="flex-none w-[170px] sm:w-[200px] lg:w-[220px]">
    ...
    <div className="group flex flex-col"
      ...
      <Link href={`/product/${product.id}`}
        ...
        <Image src={product.images[0]} alt={product.name} fill
          sizes="220px"
          className={`object-cover object-top transition-opacity duration-500 ${hovered && product.images[1] ? "opacity-0" : "opacity-100"}`} />
        {product.images[1] && (
        ...
        <button
          ...
          className="absolute top-2 right-2 w-7 h-7 bg-white/90 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10 shadow-sm">
          <Heart size={13} strokeWidth={1.5}
            className={isWishlisted(product.id) ? "fill-red-500 text-red-500" : "text-charcoal"} />
        </button>
        <AnimatePresence>
          {hovered && (
            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 5 }} transition={{ duration: 0.18 }}
              className="absolute bottom-0 left-0 right-0 bg-charcoal/90 py-2 text-center z-10">
      ...
      <div className="mt-2.5">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          {product.originalPrice && (
            <span className="font-body text-sm text-gray-400 line-through">€{product.originalPrice.toFixed(2)}</span>
          )}
        </div>
      </div>
    </div>
