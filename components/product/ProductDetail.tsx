...
export default function ProductDetail({ product, related }: Props) {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 pt-6 pb-16">
      {/* Breadcrumb */}
      ...
      <div className="grid lg:grid-cols-12 gap-6 lg:gap-12">
        ...
        <div className="lg:col-span-7 flex gap-3">
          ...
          <div className="hidden sm:flex flex-col gap-2 w-14 flex-none">
            {product.images.map((img, i) => (
              <button
                ...
                className={`relative overflow-hidden bg-bone flex-none transition-all ${
                  selectedImage === i ? "ring-1 ring-charcoal" : "opacity-50 hover:opacity-80"
                }`}
          ...
          {/* Main image */}
          <div
            className="flex-1 relative overflow-hidden bg-bone cursor-zoom-in"
            style={{ aspectRatio: "3/4" }}
              ...
              {product.images.map((_, i) => (
                <button
                  ...
                  className={`w-1.5 h-1.5 rounded-full transition-all ${
                    selectedImage === i ? "bg-charcoal" : "bg-charcoal/30"
                  }`}
                />
              ))}
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <div className="flex items-baseline gap-3 mb-5">
              ...
              {product.originalPrice && (
                <span className="font-display text-xl text-stone line-through">€{product.originalPrice}</span>
              )}
              ...
              <div className="grid grid-cols-5 sm:grid-cols-6 gap-1.5">
                {product.sizes.map((size) => (
                  <button
                    ...
                    className={`py-2.5 font-body text-xs tracking-editorial border transition-all duration-150 ${
                      selectedSize === size
            ...
            <div className="flex gap-3 mb-4">
              <div className="flex items-center border border-bone">
                <button
                  ...
                  className="px-3 py-4 hover:bg-bone transition-colors"
                  aria-label="Decrease"
                >
                  <Minus size={12} />
                </button>
              ...
              <button
                ...
                className="flex-1 font-body text-xs tracking-editorial py-4 bg-charcoal text-white hover:bg-charcoal/85 active:scale-[0.99] transition-all relative overflow-hidden"
              >
                <AnimatePresence mode="wait">
                  {added ? (
                    <motion.span key="added" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} className="block">
                      ADDED TO BAG ✓
                    </motion.span>
                  ) : (
                    <motion.span key="add" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} className="block">
                      ADD TO BAG
                    </motion.span>
                  )}
                </AnimatePresence>
              ...
              <button
                ...
                className="w-14 border border-bone flex items-center justify-center hover:border-charcoal transition-colors"
                aria-label="Wishlist"
              >
                <Heart
                  size={16}
                  strokeWidth={1.5}
                  className={`transition-colors ${isWishlisted(product.id) ? "fill-charcoal text-charcoal" : "text-charcoal"}`}
                />
              </button>
            </div>
                ...
                {detailsOpen && (
                  ...
                  >
                    {product.details.map((d) => (
                      <li key={d} className="flex items-start gap-3 font-body text-sm text-stone">
